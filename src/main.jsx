...
function App() {
 ...
 return <>
  ...
  <main>
   ...
   <section id="explore" className="shell library" aria-labelledby="library-title"><div className="section-heading"><div><p className="eyebrow">The collection</p><h2 id="library-title">Find your next rabbit hole.</h2><p>{channels.length} channels, chosen with care by Pedro.</p></div><label className="search"><Search size={23}/><span className="sr-only">Search channels, topics or ideas</span><input type="search" placeholder="What are you curious about?" value={query} onChange={e=>setQuery(e.target.value)}/></label></div><div className="filters" aria-label="Filter by topic">{topics.map(t=><button key={t} aria-pressed={topic===t} onClick={()=>setTopic(t)}>{t}</button>)}</div><p className="result-count" role="status">{filtered.length} {filtered.length===1?'channel':'channels'}{topic!=='All'?` about ${topic.toLowerCase()}`:''}{query.trim()?` matching “${query.trim()}”`:''}</p><div className="channel-grid">{filtered.map((c,i)=><article className="channel" key={c.id}><p className="eyebrow">{c.style}</p><div className="channel-heading"><span className={`monogram mark-${channels.indexOf(c)%4}`} aria-hidden="true">{c.mark}</span><h3>{c.name}</h3></div><p className="description">{summaries[c.id]}</p><button className="explore-link" onClick={()=>setSelected(c)} aria-label={`Explore channel: ${c.name}`}>Explore channel <ArrowUpRight size={19}/></button></article>)}</div>{!filtered.length&&<div className="empty"><h3>No rabbit holes here yet.</h3><p>Try another subject, a channel name, or clear your filters.</p><button className="button primary" onClick={reset}>Show all channels</button></div>}<p className="library-caption">Choose a subject you love. Let the English follow.</p></section>
  ...
  <dialog ref={dialog} className="channel-dialog" aria-labelledby="detail-title" onCancel={()=>setSelected(null)} onClick={e=>{if(e.target===dialog.current)setSelected(null);}}>{selected&&<div className="dialog-content"><button className="icon-button close" autoFocus onClick={()=>setSelected(null)} aria-label="Close channel details"><X size={24}/></button><p className="eyebrow">{selected.topics.join(' / ')}</p><h2 id="detail-title">{selected.name}</h2><p>{selected.description}</p><h3>Who is it for?</h3><p>{selected.audience}</p><div className="learning-tip"><h3>Try this with your English</h3><p>{selected.englishTip}</p></div><a className="button primary" href={selected.url} target="_blank" rel="noopener noreferrer">Visit YouTube channel <ArrowUpRight size={21}/></a><p className="detail-credit">Channel description from Pedro’s English YouTube Library. Learning activity added for the website.</p></div>}</dialog>
 </>;
}
